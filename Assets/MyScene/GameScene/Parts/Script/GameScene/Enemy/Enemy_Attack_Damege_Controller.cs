    ...
    void OnTriggerEnter(Collider collider)
    {
        ...
        switch (enumTag)
        {
            case MyEnumerator.EnumeratorTag.UnityChanWeapon:             //UnityChanの武器に接触した場合
                ...
                break;
            case MyEnumerator.EnumeratorTag.UnityChan_MagicAttack:
                {
                    GameObject obj = collider.gameObject;
                    MagicDamage magicDamage = obj.GetComponent<MagicDamage>();

                    int damage = ((IMagicDamage)magicDamage).IMagicDamage();
                    int confirmedDamage = m_Condition.DamagedPoint(damage);
                    m_ThisHPBar.HitPointBarValue = (float)m_Condition.Life / m_Condition.MaxLife;
                    MyDebug.MyDebugLog.Log(this.name + " : Magic 被ダメージ " + confirmedDamage + " : " + (float)m_Condition.Life / m_Condition.MaxLife + "  " + damage);
                }
                break;
            default:
